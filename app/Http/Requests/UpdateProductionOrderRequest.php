...
class UpdateProductionOrderRequest extends FormRequest
{
    ...
    {
        $productionOrderId = $this->route('production_order')?->getKey();

        return [
            'order_number' => ['required', 'string', 'max:50', Rule::unique('production_orders', 'order_number')->ignore($productionOrderId)],
